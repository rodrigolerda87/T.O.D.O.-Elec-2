...
import {
  ...
  calcularAmpacidad, METODOS_INSTALACION, FACTOR_TEMPERATURA,
  calcularLlenadoCano, CANOS_IRAM, DIAMETRO_CABLE_PVC,
  calcularPuestaTierra, TIPOS_SUELO, TIPOS_JABALINA,
  calcularContactorMotor, FACTOR_ARRANQUE,
...
const state = {
  ...
  // Cálculos: Ampacidad del cable
  ampacidad: { aislacion: 'pvc', metodoId: 'B1', temperatura: 40, circuitosAgrupados: 1, corrienteRequeridaA: 20 },

  // Cálculos: Diámetro de caño
  cano: { conductores: [{ s: 2.5, cantidad: 3 }], canoNombre: '3/4" (20 mm)' },

  // Cálculos: Puesta a tierra
  tierra: { tipoSueloId: 'arcilloso_humedo', tipoJabalinaId: 'estandar', objetivoOhm: 10, seccionFaseMm2: 10 },

  // Cálculos: Contactor / motor
  motor: { tipoCircuito: 'trifasico', potenciaKw: 3.7, factorArranqueId: 'directo' },

  // Cálculos: Potencia y simultaneidad
  potencia: { cargas: [{ id: 'iluminacion', cantidad: 1 }, { id: 'tug', cantidad: 1 }] },
...
const VIEW_GROUP = {
  ...
  business: 'negocio', clients: 'negocio', catalog: 'negocio',
  'calc-caida': 'calculos', 'calc-electrif': 'calculos',
  'calc-ampacidad': 'calculos', 'calc-cano': 'calculos', 'calc-tierra': 'calculos',
  'calc-motor': 'calculos', 'calc-potencia': 'calculos', 'calc-diferencial': 'calculos',
  guias: 'guias', 'guia-detalle': 'guias',
};
const GROUP_DEFAULT = { home: 'home', presupuestos: 'builder', negocio: 'business', calculos: 'calc-caida', guias: 'guias' };
...
function renderView(name) {
  ...
  else if (name === 'calc-ampacidad') renderAmpacidad();
  else if (name === 'calc-cano') renderDiametroCano();
  else if (name === 'calc-tierra') renderPuestaTierra();
  else if (name === 'calc-motor') renderContactorMotor();
  else if (name === 'calc-potencia') renderPotenciaSimultaneidad();
  else if (name === 'calc-diferencial') renderSensibilidadDiferencial();
  else if (name === 'guias') renderGuiasLista();
  else if (name === 'guia-detalle') renderGuiaDetalle();
}

const NEGOCIO_SUBTABS = [
  { v: 'business', label: 'Mi negocio' },
  { v: 'clients', label: 'Clientes' },
  { v: 'catalog', label: 'Precios AAIERIC' },
];
const CALCULOS_SUBTABS = [
  { v: 'calc-caida', label: 'Caída de Tensión' },
  { v: 'calc-electrif', label: 'Grado de Electrificación' },
  { v: 'calc-ampacidad', label: 'Ampacidad del cable' },
  { v: 'calc-cano', label: 'Diámetro de caño' },
  { v: 'calc-tierra', label: 'Puesta a tierra' },
  { v: 'calc-motor', label: 'Contactor / motor' },
  { v: 'calc-potencia', label: 'Potencia y simultaneidad' },
  { v: 'calc-diferencial', label: 'Diferencial' },
];
...
function renderClients() {
  ...
  el.innerHTML = `
    ${subtabsHtml(NEGOCIO_SUBTABS, 'clients')}
    <div class="page-head">
...
function renderCatalog() {
  ...
  el.innerHTML = `
    ${subtabsHtml(NEGOCIO_SUBTABS, 'catalog')}
    <div class="page-head">
      <div>
...
function renderBusiness() {
  ...
  const p = state.prefs;
  el.innerHTML = `
    ${subtabsHtml(NEGOCIO_SUBTABS, 'business')}
    <div class="page-head">
...
const INSTITUCIONALES = [
  { nombre: 'AEA · Reglamentación 90364', img: './assets/branding/aea-banner.png', url: './docs/AEA_90364.pdf' },
  { nombre: 'AAIERIC', img: './assets/branding/aaieric-banner.png', url: 'https://aaieric.org.ar/' },
  { nombre: 'Prysmian · Cables', img: './assets/branding/prysmian-banner.png', url: 'https://ar.prysmian.com/es' },
  { nombre: 'Schneider Electric', img: './assets/branding/schneider-banner.png', url: 'https://www.se.com/ar/es/' },
...
function acSlide(item, i, linked, compact) {
  ...
  const inner = compact
    ? `<div class="ac-slide-img" data-fallback="${esc(item.nombre)}">${img}</div>`
    : `<div class="ac-slide-img" data-fallback="${esc(item.nombre)}">${img}</div><div class="ac-slide-label">${esc(item.nombre)}</div>`;
...
function autoCarouselHtml(id, items, linked, compact) {
  return `
    <div class="auto-carousel ${compact ? 'auto-carousel-compact' : ''}" id="${id}">
      <div class="ac-track">${items.map((it, i) => acSlide(it, i, linked, compact)).join('')}</div>
...
function renderHome() {
  ...
  const nombre = state.business.responsable ? state.business.responsable.split(' ')[0] : '';
  el.innerHTML = `
    <h1 class="home-greeting" style="margin-top:10px;">Hola${nombre ? ', ' + esc(nombre) : ''}</h1>

    <div class="card-title" style="margin-top:6px;">¿Qué vas a hacer hoy?</div>
    <div class="home-grid">
      ${HOME_ACCESOS.map(a => `
    ...
    ${autoCarouselHtml('carousel-institucional', INSTITUCIONALES, true, true)}
    ${autoCarouselHtml('carousel-comercios', COMERCIOS_LOCALES, false, true)}
  `;
  wireGoto(el);
...
function renderCaidaTension() {
  ...
  el.innerHTML = `
    ${subtabsHtml(CALCULOS_SUBTABS, 'calc-caida')}
    <div class="page-head">
...
// ---------------------------------------------------------------------
// VIEW: Cálculos · Ampacidad del cable
// ---------------------------------------------------------------------
function renderAmpacidad() {
  const el = document.getElementById('view-calc-ampacidad');
  const f = state.ampacidad;
  el.innerHTML = `
    ${subtabsHtml(CALCULOS_SUBTABS, 'calc-ampacidad')}
    <div class="page-head">
      <div><h1 class="page-title">Ampacidad del cable</h1><p class="page-sub">Corriente máxima admisible (Iz) según AEA 90364-7-770 / IEC 60364-5-52.</p></div>
    </div>
    <div class="builder-grid">
      <div class="card">
        <div class="card-title">Condiciones de instalación</div>
        <div class="field"><label>Aislación</label>
          <select id="am-aislacion">
            <option value="pvc" ${f.aislacion === 'pvc' ? 'selected' : ''}>PVC (70°C) — estándar</option>
            <option value="xlpe" ${f.aislacion === 'xlpe' ? 'selected' : ''}>XLPE (90°C) — industrial</option>
          </select>
        </div>
        <div class="field"><label>Método de instalación</label>
          <select id="am-metodo">
            ${METODOS_INSTALACION.map(m => `<option value="${m.id}" ${m.id === f.metodoId ? 'selected' : ''}>${esc(m.nombre)}</option>`).join('')}
          </select>
        </div>
        <div class="field"><label>Temperatura ambiente</label>
          <select id="am-temp">
            ${[25, 30, 35, 40, 45, 50].map(t => `<option value="${t}" ${t === f.temperatura ? 'selected' : ''}>${t}°C${t === 40 ? ' (referencia AEA)' : ''}</option>`).join('')}
          </select>
        </div>
        <div class="field"><label>Circuitos agrupados en la misma cañería</label><input id="am-circuitos" type="number" min="1" max="12" value="${f.circuitosAgrupados}"></div>
        <div class="field"><label>Corriente que necesita el circuito (A)</label><input id="am-corriente" type="number" min="0" value="${f.corrienteRequeridaA}"></div>
        <div class="hint">Máx. 3 circuitos de uso general por cañería (AEA 770.11) — para más, separá en cañerías propias.</div>
      </div>
      <div class="card">
        <div class="card-title">Resultado</div>
        <div id="am-result">${ampacidadResultHtml(calcularAmpacidad(f))}</div>
      </div>
    </div>
  `;
  wireGoto(el);
  const update = () => { document.getElementById('am-result').innerHTML = ampacidadResultHtml(calcularAmpacidad(f)); };
  el.querySelector('#am-aislacion').onchange = e => { f.aislacion = e.target.value; update(); };
  el.querySelector('#am-metodo').onchange = e => { f.metodoId = e.target.value; update(); };
  el.querySelector('#am-temp').onchange = e => { f.temperatura = Number(e.target.value); update(); };
  el.querySelector('#am-circuitos').oninput = e => { f.circuitosAgrupados = Number(e.target.value) || 1; update(); };
  el.querySelector('#am-corriente').oninput = e => { f.corrienteRequeridaA = Number(e.target.value) || 0; update(); };
}

function ampacidadResultHtml(r) {
  return `
    <div class="kv"><span>Factor de corrección total</span><b class="mono">×${r.factorTotal.toFixed(2)}</b></div>
    <div class="hint" style="margin-bottom:10px;">Método ${r.metodo.factor} · Temperatura ${r.fTemp} · Agrupamiento ${r.fAgrup}</div>
    <div class="kv"><span>Sección sugerida</span><b class="mono">${r.sugerida.s} mm² (Iz corregida ${r.sugerida.izCorregida.toFixed(1)} A)</b></div>
    <div class="kv"><span>Disyuntor máximo</span><b class="mono">${r.sugerida.disyMax} A</b></div>
    <div class="card-title" style="margin-top:14px;">Tabla completa</div>
    <div style="max-height:260px;overflow:auto;">
      <table style="width:100%;font-size:12.5px;border-collapse:collapse;">
        <thead><tr style="text-align:left;color:var(--text-dim);"><th style="padding:4px 6px;">mm²</th><th style="padding:4px 6px;">Iz tabla</th><th style="padding:4px 6px;">Iz corregida</th><th style="padding:4px 6px;">Disyuntor máx.</th></tr></thead>
        <tbody>
          ${r.filas.map(row => `<tr style="border-top:1px solid var(--border);${row.s === r.sugerida.s ? 'background:var(--amber-soft);' : ''}"><td style="padding:4px 6px;" class="mono">${row.s}</td><td style="padding:4px 6px;" class="mono">${row.izTabla.toFixed(1)} A</td><td style="padding:4px 6px;" class="mono">${row.izCorregida.toFixed(1)} A</td><td style="padding:4px 6px;" class="mono">${row.disyMax} A</td></tr>`).join('')}
        </tbody>
      </table>
    </div>
    <p class="hint" style="margin-top:10px;">Cálculo orientativo (AEA 90364-7-770 Tabla 770.12.I, cobre, método B1 a 40°C como base). La sección final también debe verificar caída de tensión.</p>
  `;
}

// ---------------------------------------------------------------------
// VIEW: Cálculos · Diámetro de caño
// ---------------------------------------------------------------------
function renderDiametroCano() {
  const el = document.getElementById('view-calc-cano');
  const f = state.cano;
  el.innerHTML = `
    ${subtabsHtml(CALCULOS_SUBTABS, 'calc-cano')}
    <div class="page-head">
      <div><h1 class="page-title">Diámetro de caño</h1><p class="page-sub">Llenado máximo 35% según AEA 90364-7-771 §771.12.3.13.</p></div>
    </div>
    <div class="builder-grid">
      <div class="card">
        <div class="card-title">Conductores a pasar</div>
        <div id="cano-rows"></div>
        <button class="btn btn-sm" id="cano-add" style="margin-top:8px;">+ Agregar cable</button>
        <div class="field" style="margin-top:14px;"><label>Caño a verificar</label>
          <select id="cano-select">
            ${CANOS_IRAM.map(c => `<option value="${esc(c.nombre)}" ${c.nombre === f.canoNombre ? 'selected' : ''}>${esc(c.nombre)}</option>`).join('')}
          </select>
        </div>
      </div>
      <div class="card">
        <div class="card-title">Resultado</div>
        <div id="cano-result">${canoResultHtml(calcularLlenadoCano(f))}</div>
      </div>
    </div>
  `;
  wireGoto(el);
  ...
  function renderRows() {
    document.getElementById('cano-rows').innerHTML = f.conductores.map((c, i) => `
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px;" data-row="${i}">
        <select class="cano-s" style="flex:2;">
          ${DIAMETRO_CABLE_PVC.map(d => `<option value="${d.s}" ${d.s === c.s ? 'selected' : ''}>${d.s} mm²</option>`).join('')}
        </select>
        <input class="cano-cant" type="number" min="1" value="${c.cantidad}" style="flex:1;">
        <button class="icon-btn cano-del" title="Quitar">${ICON_TRASH}</button>
      </div>
    `).join('');
    document.querySelectorAll('#cano-rows [data-row]').forEach(row => {
      const i = Number(row.dataset.row);
      row.querySelector('.cano-s').onchange = e => { f.conductores[i].s = Number(e.target.value); update(); };
      row.querySelector('.cano-cant').oninput = e => { f.conductores[i].cantidad = Number(e.target.value) || 1; update(); };
      row.querySelector('.cano-del').onclick = () => { f.conductores.splice(i, 1); renderRows(); update(); };
    });
  }
  function update() { document.getElementById('cano-result').innerHTML = canoResultHtml(calcularLlenadoCano(f)); }

  renderRows();
  document.getElementById('cano-add').onclick = () => { f.conductores.push({ s: 2.5, cantidad: 1 }); renderRows(); update(); };
  document.getElementById('cano-select').onchange = e => { f.canoNombre = e.target.value; update(); };
}

function canoResultHtml(r) {
  return `
    <div class="kv"><span>Ocupación</span><b class="mono">${r.porcentaje.toFixed(1)}% de ${r.cano.nombre}</b></div>
    <div class="kv"><span>Cumple 35% máximo</span>${r.cumple ? '<span class="badge badge-success">Sí</span>' : '<span class="badge badge-danger">No</span>'}</div>
    <div class="kv"><span>Área ocupada / límite</span><b class="mono">${r.areaOcupada.toFixed(1)} / ${r.limiteMm2.toFixed(1)} mm²</b></div>
    <div class="kv"><span>Cantidad de cables</span><b class="mono">${r.cantidadCables}</b></div>
    <div class="kv"><span>Caño mínimo sugerido</span><b class="mono">${r.canoSugerido.nombre}</b></div>
    ${r.riesgoAtasco ? '<div class="kv"><span>Riesgo de atasco (jam)</span><span class="badge badge-danger">Cable grueso para este caño</span></div>' : ''}
    <p class="hint" style="margin-top:10px;">AEA 90364-7-771 §771.12.3.13: hasta 35% de ocupación (distinto del 40% del NEC de EE.UU.). Diámetro exterior de cables PVC según IRAM NM 247-3, orientativo.</p>
  `;
}

// ---------------------------------------------------------------------
// VIEW: Cálculos · Puesta a tierra
// ---------------------------------------------------------------------
function renderPuestaTierra() {
  const el = document.getElementById('view-calc-tierra');
  const f = state.tierra;
  el.innerHTML = `
    ${subtabsHtml(CALCULOS_SUBTABS, 'calc-tierra')}
    <div class="page-head">
      <div><h1 class="page-title">Puesta a tierra</h1><p class="page-sub">Cantidad de jabalinas según AEA 90364-5-54.</p></div>
    </div>
    <div class="builder-grid">
      <div class="card">
        <div class="card-title">Datos del terreno</div>
        <div class="field"><label>Tipo de suelo</label>
          <select id="pt-suelo">
            ${TIPOS_SUELO.map(s => `<option value="${s.id}" ${s.id === f.tipoSueloId ? 'selected' : ''}>${esc(s.nombre)} (ρ≈${s.rho} Ω·m)</option>`).join('')}
          </select>
        </div>
        <div class="field"><label>Tipo de jabalina</label>
          <select id="pt-jabalina">
            ${TIPOS_JABALINA.map(j => `<option value="${j.id}" ${j.id === f.tipoJabalinaId ? 'selected' : ''}>${esc(j.nombre)}</option>`).join('')}
          </select>
        </div>
        <div class="field"><label>Resistencia objetivo (Ω)</label><input id="pt-objetivo" type="number" min="1" value="${f.objetivoOhm}"></div>
        <div class="field"><label>Sección de la fase principal (mm²)</label>
          <select id="pt-fase">
            ${[4, 6, 10, 16, 25, 35, 50, 70].map(s => `<option value="${s}" ${s === f.seccionFaseMm2 ? 'selected' : ''}>${s} mm²</option>`).join('')}
          </select>
        </div>
        <div class="hint">Límite formal AEA: ≤40Ω con diferencial de 30mA. Objetivo de ingeniería recomendado: ≤10Ω residencial.</div>
      </div>
      <div class="card">
        <div class="card-title">Resultado</div>
        <div id="pt-result">${tierraResultHtml(calcularPuestaTierra(f))}</div>
      </div>
    </div>
  `;
  wireGoto(el);
  const update = () => { document.getElementById('pt-result').innerHTML = tierraResultHtml(calcularPuestaTierra(f)); };
  el.querySelector('#pt-suelo').onchange = e => { f.tipoSueloId = e.target.value; update(); };
  el.querySelector('#pt-jabalina').onchange = e => { f.tipoJabalinaId = e.target.value; update(); };
  el.querySelector('#pt-objetivo').oninput = e => { f.objetivoOhm = Number(e.target.value) || 10; update(); };
  el.querySelector('#pt-fase').onchange = e => { f.seccionFaseMm2 = Number(e.target.value); update(); };
}

function tierraResultHtml(r) {
  return `
    <div class="kv"><span>Jabalinas necesarias</span><b class="mono">${r.n}</b></div>
    <div class="kv"><span>Resistencia estimada</span><b class="mono">${r.resistenciaEstimada.toFixed(1)} Ω</b></div>
    <div class="kv"><span>Cumple objetivo (${r.n >= 1 ? 'elegido' : ''})</span>${r.cumpleObjetivo ? '<span class="badge badge-success">Sí</span>' : '<span class="badge badge-danger">No</span>'}</div>
    <div class="kv"><span>Cumple límite formal AEA (40Ω)</span>${r.cumpleAea40 ? '<span class="badge badge-success">Sí</span>' : '<span class="badge badge-danger">No</span>'}</div>
    <div class="kv"><span>Separación mínima entre jabalinas</span><b class="mono">${r.separacionMinM.toFixed(1)} m</b></div>
    <div class="kv"><span>Sección conductor PE</span><b class="mono">${r.seccionPE} mm²</b></div>
    <p class="hint" style="margin-top:10px;">Fórmula de Dwight + factor de paralelo de Tagg, suelo homogéneo asumido. Este cálculo teórico suele ser pesimista — el valor medido con telurímetro después de instalar casi siempre da mejor. Es un pre-dimensionamiento: esa medición post-instalación es la que vale para la habilitación (AEA 90364-5-54).</p>
  `;
}

// ---------------------------------------------------------------------
// VIEW: Cálculos · Contactor / térmica para motores
// ---------------------------------------------------------------------
function renderContactorMotor() {
  const el = document.getElementById('view-calc-motor');
  const f = state.motor;
  el.innerHTML = `
    ${subtabsHtml(CALCULOS_SUBTABS, 'calc-motor')}
    <div class="page-head">
      <div><h1 class="page-title">Contactor / térmica para motores</h1><p class="page-sub">Corriente nominal, arranque y calibre sugerido.</p></div>
    </div>
    <div class="builder-grid">
      <div class="card">
        <div class="card-title">Datos del motor</div>
        <div class="field"><label>Tipo de circuito</label>
          <select id="mo-tipo">
            <option value="monofasico" ${f.tipoCircuito === 'monofasico' ? 'selected' : ''}>Monofásico 220V</option>
            <option value="trifasico" ${f.tipoCircuito === 'trifasico' ? 'selected' : ''}>Trifásico 380V</option>
        ...
        <div class="field"><label>Potencia (kW)</label><input id="mo-kw" type="number" min="0" step="0.1" value="${f.potenciaKw}"></div>
        <div class="field"><label>Tipo de arranque</label>
          <select id="mo-arranque">
            ${FACTOR_ARRANQUE.map(a => `<option value="${a.id}" ${a.id === f.factorArranqueId ? 'selected' : ''}>${esc(a.nombre)}</option>`).join('')}
          </select>
        </div>
      </div>
      <div class="card">
        <div class="card-title">Resultado</div>
        <div id="mo-result">${motorResultHtml(calcularContactorMotor(f))}</div>
      </div>
    </div>
  `;
  wireGoto(el);
  const update = () => { document.getElementById('mo-result').innerHTML = motorResultHtml(calcularContactorMotor(f)); };
  el.querySelector('#mo-tipo').onchange = e => { f.tipoCircuito = e.target.value; update(); };
  el.querySelector('#mo-kw').oninput = e => { f.potenciaKw = Number(e.target.value) || 0; update(); };
  el.querySelector('#mo-arranque').onchange = e => { f.factorArranqueId = e.target.value; update(); };
}

function motorResultHtml(r) {
  return `
    <div class="kv"><span>Corriente nominal (In)</span><b class="mono">${r.fila.a} A</b></div>
    <div class="kv"><span>Corriente de arranque estimada</span><b class="mono">${r.corrienteArranque.toFixed(1)} A</b></div>
    <div class="kv"><span>Contactor sugerido (AC-3)</span><b class="mono">≥${r.contactorSugeridoA} A</b></div>
    <div class="kv"><span>Guardamotor / térmica regulable</span><b class="mono">${r.guardamotorRangoA.min}–${r.guardamotorRangoA.max} A</b></div>
    <div class="kv"><span>Sección de cable sugerida</span><b class="mono">${r.cableSugerido.s} mm²</b></div>
    <p class="hint" style="margin-top:10px;">Corrientes de referencia de catálogo (cos φ ≈ 0.8-0.85). Verificá siempre contra la placa del motor real.</p>
  `;
}

// ---------------------------------------------------------------------
// VIEW: Cálculos · Potencia total y factor de simultaneidad
// ---------------------------------------------------------------------
function renderPotenciaSimultaneidad() {
  const el = document.getElementById('view-calc-potencia');
  const f = state.potencia;
  el.innerHTML = `
    ${subtabsHtml(CALCULOS_SUBTABS, 'calc-potencia')}
    <div class="page-head">
      <div><h1 class="page-title">Potencia y simultaneidad</h1><p class="page-sub">Potencia instalada vs. potencia simultánea real.</p></div>
    </div>
    <div class="builder-grid">
      <div class="card">
        <div class="card-title">Cargas de la vivienda</div>
        <div id="pot-rows"></div>
        <button class="btn btn-sm" id="pot-add" style="margin-top:8px;">+ Agregar carga</button>
      </div>
      <div class="card">
        <div class="card-title">Resultado</div>
        <div id="pot-result">${potenciaResultHtml(calcularPotenciaSimultaneidad(f.cargas))}</div>
      </div>
    </div>
  `;
  wireGoto(el);
  ...
  function renderRows() {
    document.getElementById('pot-rows').innerHTML = f.cargas.map((c, i) => `
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px;" data-row="${i}">
        <select class="pot-id" style="flex:2;">
          ${CARGAS_TIPICAS.map(t => `<option value="${t.id}" ${t.id === c.id ? 'selected' : ''}>${esc(t.nombre)} (${t.w}W)</option>`).join('')}
        </select>
        <input class="pot-cant" type="number" min="1" value="${c.cantidad}" style="flex:1;">
        <button class="icon-btn pot-del" title="Quitar">${ICON_TRASH}</button>
      </div>
    `).join('');
    document.querySelectorAll('#pot-rows [data-row]').forEach(row => {
      const i = Number(row.dataset.row);
      row.querySelector('.pot-id').onchange = e => { f.cargas[i].id = e.target.value; update(); };
      row.querySelector('.pot-cant').oninput = e => { f.cargas[i].cantidad = Number(e.target.value) || 1; update(); };
      row.querySelector('.pot-del').onclick = () => { f.cargas.splice(i, 1); renderRows(); update(); };
    });
  }
  function update() { document.getElementById('pot-result').innerHTML = potenciaResultHtml(calcularPotenciaSimultaneidad(f.cargas)); }

  renderRows();
  document.getElementById('pot-add').onclick = () => { f.cargas.push({ id: 'iluminacion', cantidad: 1 }); renderRows(); update(); };
}

function potenciaResultHtml(r) {
  return `
    <div class="kv"><span>Potencia instalada</span><b class="mono">${r.potenciaInstalada.toFixed(0)} W</b></div>
    <div class="kv"><span>Potencia simultánea (real)</span><b class="mono">${r.potenciaSimultanea.toFixed(0)} W</b></div>
    <div class="kv"><span>Corriente estimada (monofásico)</span><b class="mono">${r.corrienteMono.toFixed(1)} A</b></div>
    <div class="kv"><span>Corriente estimada (trifásico)</span><b class="mono">${r.corrienteTri.toFixed(1)} A</b></div>
    <p class="hint" style="margin-top:10px;">Factores de simultaneidad orientativos por tipo de carga. Para el interruptor general, usá siempre la potencia simultánea, no la instalada.</p>
  `;
}

// ---------------------------------------------------------------------
// VIEW: Cálculos · Sensibilidad de diferencial
// ---------------------------------------------------------------------
function renderSensibilidadDiferencial() {
  const el = document.getElementById('view-calc-diferencial');
  el.innerHTML = `
    ${subtabsHtml(CALCULOS_SUBTABS, 'calc-diferencial')}
    <div class="page-head">
      <div><h1 class="page-title">Sensibilidad de diferencial</h1><p class="page-sub">Qué IΔn usar según la aplicación.</p></div>
    </div>
    <div class="card">
      ${APLICACIONES_DIFERENCIAL.map(a => `
        <div class="list-row">
          <div>
            <div class="title">${esc(a.nombre)}</div>
            <div class="meta">${esc(a.motivo)}</div>
          </div>
          <span class="badge ${a.mA <= 30 ? 'badge-success' : 'badge-muted'}">${a.mA} mA</span>
        </div>
      `).join('')}
    </div>
    <p class="hint" style="margin-top:10px;">30mA protege personas (obligatorio en tomas y locales húmedos). 300mA es selectivo/protección contra incendio, no protege personas por sí solo — siempre debe haber 30mA aguas abajo en los circuitos de uso general.</p>
  `;
  wireGoto(el);
}
...
function renderElectrifLista() {
  ...
  el.innerHTML = `
    ${subtabsHtml(CALCULOS_SUBTABS, 'calc-electrif')}
    <div class="page-head">
...
function renderElectrifEditor() {
  ...
  document.getElementById('plano-file').onchange = async (e) => {
    ...
    try {
      ...
      console.error(err);
      toast(`No se pudo leer ese archivo (${err && err.message ? err.message : 'error desconocido'}). Probá con otra foto, imagen o PDF.`, 'error');
